...
// PASTE YOUR HTML GENERATORS HERE (buildAssetsHtml, cbomToHtml, scanResultToHtml)

async function processAndSendReport(params) {
    const { email, targetDomainId, includeSections, scheduleName } = params;
    let attachments = [];

    // 1. Resolve Domains
    let domainIds = targetDomainId === 'all' ? await Asset.distinct('domainId') : [targetDomainId];
    ...
    for (const dId of domainIds) {
        ...
        if (includeSections.assets) {
            ...
            if (enriched.length > 0) {
                const html = buildAssetsHtml(enriched, scheduleName);
                attachments.push({ filename: `${dName}_Assets.pdf`, content: await generatePdf(html) });
            }
        }

        // --- MODULE 2: CBOM (Robust Linkage) ---
        if (includeSections.cboms) {
            const lastScan = await Scan.findOne({ ...dQuery, status: 'completed' }).sort({ createdAt: -1 });
            if (lastScan) {
                const cbomData = await Cbom.findOne({ scanId: lastScan._id }).lean();
                if (cbomData) {
                    const html = cbomToHtml(cbomData);
                    attachments.push({ filename: `${dName}_CBOM_Registry.pdf`, content: await generatePdf(html) });
                }
            }
        }

        // --- MODULE 3: SCAN RESULTS ---
        if (includeSections.scanResults) {
            const summary = await computeDomainSummary(dId);
            const latestScan = await Scan.findOne({ ...dQuery, status: 'completed' }).sort({ createdAt: -1 });
            if (latestScan && summary) {
                const results = await ScanResult.find({ scanId: latestScan._id }).populate("assetId").lean();
                const recommendations = await Recommendation.find({ scanResultId: { $in: results.map(r => r._id) } }).lean();
                const cryptoInventory = [...new Set(results.map(r => r.negotiated?.cipher).filter(Boolean))];
                
                const html = scanResultToHtml(summary, cryptoInventory, results, recommendations, scheduleName);
                attachments.push({ filename: `${dName}_Posture_Analysis.pdf`, content: await generatePdf(html) });
            }
        }
    }

    if (attachments.length > 0) {
        await sendReportEmail(email, scheduleName, attachments);
    }
}
